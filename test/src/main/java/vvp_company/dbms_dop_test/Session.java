...
public class Session implements AutoCloseable {
    ...
    private final long WS_TIMEOUT = 5000;
    ...
    public void begin(String isolation) throws Exception {
        ...
        var resp = ws.sendAndWaitResponse(cmd, WS_TIMEOUT);
    ...
    public void commit() throws Exception {
        var resp = ws.sendAndWaitResponse("commit", WS_TIMEOUT);
        if (!resp.equals("OK")) throw new RuntimeException("Commit failed: " + resp);
        history.add("COMMIT");
    }

    public void abort() throws Exception {
        var resp = ws.sendAndWaitResponse("abort", WS_TIMEOUT);
